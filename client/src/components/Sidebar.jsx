       ...
       <div className=" flex  justify-between items-center">
        ...
        {userDummyData.map((user,idx)=>(
            ...
            className={` relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}  key={idx}>
                <img src={user?.profilePic || user.avatar_icon} alt="Profilepic" className="w-[35px] aspect-[1/1] rounded-full" />
                <div className="flex flex-col leading-5">
                <p>{user.fullName}</p>
                {
                    idx<3 ? <span className="text-sm text-green-400">Online</span> :
                    <span className="text-sm text-neutral-400">Offline</span>
                }
                </div>
                {idx>2 && <p className="absolute right-4 bg-violet-500/50 rounded-full h-5 w-5 flex items-center justify-center">{idx}</p>}
            </div>
        ))}
        </div>
